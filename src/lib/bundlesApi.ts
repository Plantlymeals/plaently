...
export async function fetchPublishedBundles(): Promise<BundleRow[]> {
  try {
    const url = `${import.meta.env.VITE_SUPABASE_URL}/rest/v1/bundles?select=id,name,is_mixable,components,meal_count,price,per_meal_price,badge,description,sort_order,is_published,shopify_product_id&is_published=eq.true&order=sort_order.asc`;
    ...
    try {
      const { data } = await supabase
        .from("bundles")
        .select("id,name,is_mixable,components,meal_count,price,per_meal_price,badge,description,sort_order,is_published,shopify_product_id")
